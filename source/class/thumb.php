 ...
 *  
 *  http://www.tuibian.com/show-347-1.html
 *
 * @author 偶然
...
class ThumbHandler{
    ...
    function _createMaskImg($src)
    {
		...
		//如果水印大于原图
		//if( $this->mask_w > $this->dst_w || $this->mask_h > $this->dst_h ) return;
		
		//创建一张空白图像
        $cut = imagecreatetruecolor($this->mask_w, $this->mask_h);
		
		//将源图拷贝至图像
        imagecopy($cut, $src, 0, 0, $this->mask_pos_x, $this->mask_pos_y, $this->mask_w, $this->mask_h);
		
		//将水印与图像合成
        imagecopy($cut, $this->h_mask, 0, 0, 0, 0, $this->mask_w, $this->mask_h);
    ...
    function _output()
    {
        ...
        if(function_exists($func_name))
        {
			...
			//修正 png 图片质量，2010/3/4
			if ($img_type == 'png') $this->img_display_quality = 9;
        
	        //增加渐进式图片载入，2013/9/22
	        if ($img_type == 'jpg') imageinterlace($this->h_dst, 1);
    ...
    function _countMaskPos()
    {
        if($this->_isFull())
        {
            ...
            switch($this->mask_position)
            {
                case 1:
                    // 左上
                    $this->mask_pos_x = $this->mask_offset_x + $this->img_border_size + 10;
                    $this->mask_pos_y = $this->mask_offset_y + $this->img_border_size + 10;
                    break;
				...
				case 5:
					//居中
					$this->mask_pos_x = ($this->dst_w - $this->mask_w - $this->mask_offset_x - $this->img_border_size)/2;
					$this->mask_pos_y = ($this->dst_h - $this->mask_h - $this->mask_offset_y - $this->img_border_size)/2;
 					break;
        ...
        if(2 == $num){
            ...
            switch($this->cut_type)
            {
                ...
                case 3:
					...
					//改变后的图象的比例
					$resize = $fill_w / $fill_h;
					
					//实际图象的比例
					$ratio = $this->src_w / $this->src_h;

					//高度优先
					if( $ratio >= $resize ){
					
						//$newimg = imagecreatetruecolor( $fill_w, $fill_h );
						//imagecopyresampled($newimg, $this->h_src, 0, 0, 0, 0, $fill_w,$fill_h, (($this->src_h)*$resize), $this->src_h);
						//ImageJpeg ($newimg,$this->h_dst);
						
						//exit;
						
						//缩放比
						$zoomed = $fill_h / $this->src_h;
						
						$this->copy_h = $this->src_h;
                        $this->copy_w = $this->src_h * $resize;
                        
						/*
                        var_dump( $zoomed );
                        var_dump( $fill_w, $fill_h );
                        var_dump( $this->copy_w, $this->copy_h );
                        var_dump( ( $this->src_w - $fill_w ) / 2 );
                        exit;
						*/
						...
						//$this->src_x , $this->src_y
						//裁切宽度（原始尺寸 * 缩放比 - 目标尺寸 / 2（居中裁切） ）
						$this->setSrcCutPosition( ( $this->src_w * $zoomed - $fill_w ) / 2, 0 );
						
						//$this->fill_w , $this->fill_h
                        $this->setRectangleCut( $fill_w, $fill_h );
                        
                       //var_dump( $fill_w * ( $fill_h / $this->src_h ) );
                      // exit;
					...
					if( $ratio < $resize ){
						...
						//$newimg = imagecreatetruecolor( $fill_w, $fill_h );
						//imagecopyresampled($newimg, $this->h_src, 0, 0, 0, 110, $fill_w, $fill_h, $this->src_w, (($this->src_w)/$resize_ratio));
						//ImageJpeg ($newimg,$this->h_dst);	
						
						//缩放比
						$zoomed = $fill_w / $this->src_w;
						
                        $this->copy_w = $this->src_w;
                        $this->copy_h = $this->src_w / $resize;
						
						/*
                        var_dump( $zoomed );
						var_dump( $this->src_h * $zoomed  );
						var_dump( $fill_h  );
						var_dump( ( $this->src_h * $zoomed - $fill_h ) / 2 );
                        exit;
 						*/
                        
						//$this->src_x , $this->src_y
						//裁切高度（原始尺寸 * 缩放比 - 目标尺寸 / 2（居中裁切） ）
						$this->setSrcCutPosition( 0, ( $this->src_h * $zoomed - $fill_h ) / 2 );
						
						//$this->fill_w , $this->fill_h
                        $this->setRectangleCut( $fill_w, $fill_h );
                        
                       //var_dump($this->copy_w,$this->copy_h,  $this->copy_h *  $fill_h / $this->src_h );
                       //exit;
