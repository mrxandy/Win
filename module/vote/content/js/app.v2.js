...
Mo.reader(function(){
	...
	var doValid = function( form ){
		...
		//显示匿名信息框
		if( Mo('*[data-object=dialog]').size() ){
			Mo('*[data-object=dialog]').show();
		}
		...
		if( result == false ){
			...
			Mo("form[data-appid=vote] button[type=submit]").each(function( ){
				Mo( this ).attr({"name" : Mo( this ).attr("data-name") });
			}).enabled();
	...
	Mo("form[data-appid=vote] button[type=submit]").bind('click',function( ){
		...
		//禁用全部按钮
		Mo("form[data-appid=vote] button[type=submit]").attr({"data-name" : this.name}).disabled();
		...
		//提交当前表单
		//Mo("form[data-appid=vote]").event('submit');
		//Mo("form[data-appid=vote]").submit();
		doValid( this.form );
	...
	Mo("form[data-appid=vote] input").bind('click',function( index, e ){
		...
		if( this.getAttribute('type') == 'checkbox' && ( minsize || maxsize ) ){
			...
			if( minsize && selected < minsize ){
				Mo.Message( 'error', groupnm + '至少要选择 '+ maxsize +' 个哦～', 3, { "unique" : "pp-message", "center" : true } );
				this.checked = true;
			}
