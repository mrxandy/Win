...
<?php
    ...
    <form name="form1" method="get" action="?">
        <span class="action">
            <button type="button" class="cancel" onclick="location.href='?s=account&q=<?php echo urlencode($_G['manager']['account']);?>';">我的试题</button>
            <button type="button" class="button" onclick="location.href='<?php echo $appid;?>.edit.php';">新增试题</button>
    ...
    <script type="text/javascript">
        ...
        Mo.reader(function(){
            ...
            if($_GET['mark']){
            ?>
			Mo.Tips( Mo.$("mark-<?php echo $_GET['mark'];?>"), event,'','点这里开始管理试题的选项组和子选项，只需几分钟即可完成你的工作','click',70,-8,{'unique':'bubble'});
            <?php
            }
            ?>
	...
	//如果没有权限，则限制为本人
	if( System :: check_func( 'test-other') == false ) $sql.=" and aid = ".$_G['manager']['id'];
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table">
        ...
        <tr class="thead">
            ...
            <td>创建者</td>
			<td>修改者</td>
            <td>选项 <var data-type="tip">编辑此表单选项组以及子选项</var></td>
    ...
    foreach( $result as $row ){
        ...
        ?>
          <tr class="<?php echo zebra( $i, array( "line" , "band" ) );?>" data-mark="<?php echo $row['id'];?>" data-edit="<?php echo $appid;?>.edit.php?action=edit&fid=<?php echo $row['id'];?>&jump={self}">
            ...
            <td><?php echo $_G['module']['test']["mode"][$config["FORM_MODE"]];?></td>
            ...
            <td id="tips-<?php echo $row['id'];?>" align="center">
                <a href="<?php echo $appid;?>.group.php?do=list&fid=<?php echo $row['id'];?>" id="mark-<?php echo $row['id'];?>" target="_dialog" title="表单选项组" data-width="90%" data-height="60%" class="control">管理选项</a>
            </td>
            <td>
	...
	if( count( $result ) == 0 ){
		echo '<tr><td colspan="16" class="notice">没有检索到相关试题，<a href="'.$appid.'.edit.php">创建一个？</a></td></tr>';
	}
        ?>
