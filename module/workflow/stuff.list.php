...
<?php
	...
	if ($action){
		...
		switch($action){
			...
			case "delete":
				...
				System :: $db -> execute("DELETE FROM `mod:workflow_stuff` WHERE id in(".$list.")");
    ...
    $sql="SELECT * FROM `mod:workflow_stuff` WHERE 1=1";
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table">
		...
		?>
              ...
              <tr class="<?php echo zebra( $i, array( "line" , "band" ) );?>" data-mark="<?php echo $row['id'];?>" data-edit='stuff.edit.php?action=edit&id=<?php echo $row['id'];?>&jump={self}' onmouseover='Mo.Tips( this, event, "tips","<img src=\"<?php echo $row["image"];?>\" style=\"max-height:300px\" />","mouseover",-20,30,{ "unique" : "signbar" });'>
                ...
                <td class="text-bold text-no"><?php echo $row['surplus'];?></td>
                <td class="text-bold text-key"><a href="stuff.expend.php?do=list&sid=<?php echo $row['id'];?>" target="_dialog" title="表单选项组" data-width="90%" data-height="60%" class="control"><?php echo $row['quantity'] - $row['surplus'];?></a></td>
                <td><?php echo $row['total'];?></td>
                <td><a href="?account=<?php echo $row["aid"];?>"><?php echo $row['account'];?></a></td>
                <td><?php echo date("Y-m-d",$row['dateline']);?></td>
				<td>
					<button type="button" class="editor" data-url="stuff.edit.php?action=edit&id=<?php echo $row['id'];?>">修改</button>
