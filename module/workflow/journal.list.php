...
<?php
	...
	if ($action){
		...
		switch($action){
			...
			case "delete":
				...
				System :: $db -> execute("DELETE FROM `mod:journal` WHERE id in(".$list.")");
    ...
    <form name="find-form" id="find-form" method="get" action="?">
      <span class="action">
        ...
        <select name="s" id="s">
            <option value="content" checked>日志内容</option>
            <option value="account">提交人</option>
        </select>
    ...
    $sql="SELECT * FROM `mod:journal` WHERE '".$_G['manager']['id']."'=uid";
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table">
		...
		?>
              ...
              <tr class="<?php echo zebra( $i, array( "line" , "band" ) );?>" data-mark="<?php echo $row['id'];?>" data-edit='journal.edit.php?action=edit&id=<?php echo $row['id'];?>&jump={self}'});'>
				...
				<td>
					<button type="button" class="editor" data-url="journal.edit.php?action=edit&id=<?php echo $row['id'];?>">修改</button>
